...
export const Settings = {
            ...
            addBtn.addEventListener('click', () => {
                ...
                state.customPlaylists.forEach(pl => {
                    ...
                    btn.onclick = () => {
                        ...
                        } else alert("Цей трек вже є у плейлисті.");
                    };
    ...
    initVolumeAndBlur() {
        const volumeBar = document.getElementById('volume-bar'); const volumeIcon = document.getElementById('volume-icon');
        if (volumeBar) {
    ...
    initSource() {
        const syncLabels = () => {
            document.querySelectorAll('input[name="music-source"]').forEach(r => { 
                const lbl = r.closest('.settings-radio-label'); if (lbl) lbl.classList.toggle('checked', r.checked); 
            });
        };
    ...
    initEQ() {
        const eqToggle = document.getElementById('eq-toggle'); const eqSliders = document.querySelectorAll('.eq-band'); const eqSelect = document.getElementById('eq-presets');
        ...
        if (eqSelect) {
            eqSelect.addEventListener('change', (e) => {
                const mode = e.target.value; let vals = [0,0,0,0,0];
                if (basePresets[mode]) vals = basePresets[mode]; else if (mode.startsWith('custom_')) vals = customEqPresets[mode.replace('custom_', '')] || [0,0,0,0,0];
                eqSliders.forEach((slider, index) => { slider.value = vals[index]; if (state.isEqEnabled) setEqGain(index, vals[index]); });
            });
        }
        ...
        eqSliders.forEach(slider => { slider.addEventListener('input', (e) => { if (eqSelect) eqSelect.value = 'manual'; setEqGain(e.target.dataset.index, parseFloat(e.target.value)); }); });
        ...
        document.getElementById('save-eq-btn')?.addEventListener('click', () => {
            ...
            const vals = Array.from(eqSliders).map(s => parseFloat(s.value)); customEqPresets[name] = vals; localStorage.setItem('customEqPresets', JSON.stringify(customEqPresets));
            renderEqOptions(`custom_${name}`); document.getElementById('custom-eq-name').value = ''; UI.showToast(`Пресет "${name}" збережено!`);
        });
    ...
    initBackgrounds() {
            ...
            bgUpload.addEventListener('change', function () {
                ...
                if (file) {
                    ...
                    reader.onload = async function (e) {
                        const dataUrl = e.target.result; const newId = "bg_" + Date.now(); await saveBgToDB(newId, dataUrl); Settings.applyBackground(dataUrl, newId);
                    }
        ...
        const currentBgId = localStorage.getItem('currentBgId'); if (currentBgId) getBgFromDB(currentBgId).then(data => this.applyBackground(data, currentBgId));
    ...
    applyBackground(dataUrl, id = null) {
        if (dataUrl) {
            ...
            if (id) {
                localStorage.setItem('currentBgId', id); this.bgHistoryIds = this.bgHistoryIds.filter(i => i !== id); this.bgHistoryIds.unshift(id);
                if (this.bgHistoryIds.length > 10) this.bgHistoryIds.pop(); localStorage.setItem('auraBgHistoryIds', JSON.stringify(this.bgHistoryIds)); this.renderBgHistory();
            }
        } else {
    ...
    async renderBgHistory() {
        const list = document.getElementById('bg-history-list'); if (!list) return;
        if (this.bgHistoryIds.length === 0) { list.innerHTML = '<p style="font-size: 12px; color: var(--text-muted); font-style: italic;">Тут будуть ваші попередні фони</p>'; return; }

        const currentRender = ++this.renderCounter; const seenDataUrls = new Set(); const uniqueIds = []; const fragment = document.createDocumentFragment();
        const idsToRender = [...this.bgHistoryIds];

        for (let id of idsToRender) {
            const dataUrl = await getBgFromDB(id); if (!dataUrl) continue;
            if (seenDataUrls.has(dataUrl)) { await deleteBgFromDB(id); continue; }
            seenDataUrls.add(dataUrl); uniqueIds.push(id);

            const item = document.createElement('div'); item.style = "position: relative; display: inline-block; margin-right: 10px; flex-shrink: 0; margin-top: 5px;";
            item.innerHTML = `<img src="${dataUrl}" style="width: 70px; height: 45px; border-radius: 8px; object-fit: cover; cursor: pointer; border: 2px solid transparent; transition: 0.2s;" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='transparent'"><div class="delete-bg-btn" style="position: absolute; top: -6px; right: -6px; background: rgba(0,0,0,0.8); color: white; border-radius: 50%; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; font-size: 10px; cursor: pointer; border: 1px solid var(--accent);" onmouseover="this.style.background='var(--accent)'" onmouseout="this.style.background='rgba(0,0,0,0.8)'" title="Видалити">&#10005;</div>`;
            item.querySelector('img').onclick = () => this.applyBackground(dataUrl, id); item.querySelector('.delete-bg-btn').onclick = (e) => { e.stopPropagation(); window.deleteBg(id); };
            fragment.appendChild(item);
        }

        if (currentRender !== this.renderCounter) return;
        list.innerHTML = ''; list.appendChild(fragment);
        if (this.bgHistoryIds.length !== uniqueIds.length) { this.bgHistoryIds = uniqueIds; localStorage.setItem('auraBgHistoryIds', JSON.stringify(this.bgHistoryIds)); }
    },
    ...
    initPlaylistModals() {
        ...
        document.getElementById('pl-modal-confirm')?.addEventListener('click', () => {
            const name = document.getElementById('pl-modal-input').value.trim(); const res = Playlists.saveNew(name);
            if (res.success) { UI.showToast(res.msg); plModal.classList.add('hidden'); document.getElementById('pl-modal-input').value = ''; } else alert(res.msg);
        });
    },

    renderRecentTracks() {
        const container = document.getElementById('recent-list'); if (!container) return; container.innerHTML = '';
        if (state.recentlyPlayed.length === 0) { container.innerHTML = '<p style="font-size: 12px; color: var(--text-muted);">Поки порожньо</p>'; return; }
        ...
        if (expandBtn) {
            ...
            newBtn.addEventListener('click', function() {
                ...
                if (list.style.maxHeight === '400px') { 
                    list.style.maxHeight = '145px'; 
                    this.innerText = 'open_in_full'; 
                } else { 
                    list.style.maxHeight = '400px'; 
                    this.innerText = 'close_fullscreen'; 
                }
            });
        }
    }
